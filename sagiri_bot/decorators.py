...
from sagiri_bot.orm.async_orm import orm, PermanentBlackList
            ...
            if member_id != -1:
                ...
                if await orm.fetchone(
                    select(
                        ...
                        PermanentBlackList.id == member_id,
                        PermanentBlackList.type == "user"
                    )
                ):
                    return None
            if group_id != -1:
                if await orm.fetchone(
                    select(
                        ...
                        PermanentBlackList.id == group_id,
                        PermanentBlackList.type == "group"
                    )
                ):
                    return None
            if asyncio.iscoroutinefunction(func):
