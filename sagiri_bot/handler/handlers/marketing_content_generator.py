...
class MarketingContentGenerator(AbstractHandler):
    ...
    async def handle(app: Ariadne, message: MessageChain, group: Group = None,
        ...
        if re.match("营销号#.*#.*#.*", message.asDisplay()):
            ...
            content = f"{somebody}{something}是怎么回事呢？{somebody}相信大家都很熟悉，" \
                      f"但是{somebody}{something}是怎么回事呢，下面就让小编带大家一起了解下吧。" \
                      f"\n{somebody}{something}，其实就是{somebody}{other_word}，" \
                      f"大家可能会很惊讶{somebody}怎么会{something}呢？但事实就是这样，小编也感到非常惊讶。" \
