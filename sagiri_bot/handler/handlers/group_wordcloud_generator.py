...
class GroupWordCloudGenerator(AbstractHandler):
    ...
    async def get_review(group: Group, member: Member, review_type: str, target: str) -> MessageItem:
        ...
        if review_type == "year":
            ...
            return MessageItem(MessageChain.create([Plain(text="Error: review_type 值非法。")]), QuoteSource())
        ...
        if not (res := list(await orm.fetchall(sql))):
            return MessageItem(MessageChain.create([Plain(text="没有你的发言记录。")]), QuoteSource())
        texts = []
        for i in res:
            if i[4]:
