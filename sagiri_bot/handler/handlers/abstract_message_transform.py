...
async def abstract_message_transformer(app: Ariadne, message: MessageChain, friend: Friend):
    if result := await AbstractMessageTransformer.handle(app, message, friend=friend):
...
async def abstract_message_transformer(app: Ariadne, message: MessageChain, group: Group, member: Member):
    if result := await AbstractMessageTransformer.handle(app, message, group=group, member=member):
