    ...
    async def handle(app: Ariadne, message: MessageChain, group: Group = None,
        ...
        if message_text.startswith("摸"):
            ...
            if len(match_elements) == 1:
                ...
                if group and member:
                    await update_user_call_count_plus(group, member, UserCalledCount.functions, "functions")
                return await AvatarFunPic.kiss(member.id if member else friend.id, int(message_text[2:]))
            ...
            if len(match_elements) == 1:
                ...
                if group and member:
                    await update_user_call_count_plus(group, member, UserCalledCount.functions, "functions")
                return await AvatarFunPic.rub(member.id if member else friend.id, int(message_text[2:]))
