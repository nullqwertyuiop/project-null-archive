...
class ChatReply(AbstractHandler):
    ...
    async def get_reply(member_id: int, group_id: int, content: str):
        if mode_now := await orm.fetchone(select(Setting.speak_mode).where(Setting.group_id == group_id)):
            ...
            if mode_now == "normal":
                ...
                text = await ChatReply.get_chat_reply(content, group_id=group_id)
            else:
    ...
    @staticmethod
    async def get_chat_reply(query: str, group_id: int) -> str:
        try:
            user_data = config.functions['tencent']
            ...
            reply = json.loads(resp.to_json_string())["Reply"].replace("腾讯小龙女", "Null").\
                replace("小龙女", "Null").replace("姑姑", "Null").replace("小姐姐", "Null").replace("姑娘", "哥哥")
            return reply
