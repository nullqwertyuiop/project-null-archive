...
from SAGIRIBOT.ORM.AsyncORM import orm, UserPermission, Setting, PermanentBlackList
        ...
        if event.member.id == get_config("HostQQ"):
            ...
            if not await get_setting(event.member.group.id, Setting.event_listener):
                ...
                try:
                    ...
                    await app.sendGroupMessage(
                        event.member.group.id, MessageChain.create([
                            Plain(text="%s 被禁言 %s。" % (event.member.name, "%02d:%02d:%02d" % (h, m, s)))
                        ])
                    )
                except AccountMuted:
                    pass
...
async def bot_group_permission_change(app: Ariadne, event: BotGroupPermissionChangeEvent):
    try:
        if event.current == MemberPerm.Administrator and event.origin == MemberPerm.Member:
            await app.sendGroupMessage(
                event.group.id, MessageChain.create([
    ...
    if await get_setting(event.group.id, Setting.anti_revoke) and event.authorId != get_config("BotQQ"):
        try:
            ...
            resended_msg = MessageChain.extend(
                MessageChain.create([Plain(text=f"{event.operator.name} 撤回了 {author_name} 的一条消息：\n\n")]),
                revoked_msg
            )
            await app.sendGroupMessage(
                event.group,
