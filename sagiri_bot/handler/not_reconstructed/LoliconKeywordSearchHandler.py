...
class LoliconKeywordSearchHandler(AbstractHandler):
    ...
    async def get_image(group: Group, member: Member, keyword: str):
        ...
        if not apikey or apikey == "loliconApiKey":
            return MessageItem(MessageChain.create([Plain(text="loliconApiKey配置错误！")]), QuoteSource())
        ...
        if result["code"] == 401:
            return MessageItem(MessageChain.create([Plain(text="错误401：APIKEY 不存在或被封禁!")]), QuoteSource())
        elif result["code"] == 403:
            return MessageItem(MessageChain.create([Plain(text="错误403：由于不规范的操作而被拒绝调用！")]), QuoteSource())
        elif result["code"] == 404:
            return MessageItem(MessageChain.create([Plain(text="错误404：找不到符合关键字的色图！")]), QuoteSource())
        elif result["code"] == 429:
            return MessageItem(MessageChain.create([Plain(text="错误429：达到调用额度限制！")]), QuoteSource())
        elif result["code"] == -1:
            return MessageItem(MessageChain.create([Plain(text="错误-1：API内部错误，请向 i@loli.best 反馈！")]), QuoteSource())
