            ...
            if not tfunc:
                ...
                if tfunc == "setu":
                    if await get_setting(group.id, Setting.setu):
                        ...
                        return MessageItem(MessageChain.create([Plain(text='该功能已关闭，请阅读文档或者使用 "/contact" 联系机器人管理员开启。')]), Normal())
    ...
    async def get_image_message(group: Group, member: Member, func: str) -> MessageItem:
        if func == "setu18":
            ...
            if r18_process == "revoke":
                return MessageItem(MessageChain.create([await ImageSenderHandler.get_pic(func)]), Revoke())
            elif r18_process == "flashImage":
                return MessageItem(MessageChain.create([(FlashImage.fromImage(await ImageSenderHandler.get_pic(func)))]), Normal())
            elif r18_process == "noProcess":
                return MessageItem(MessageChain.create([await ImageSenderHandler.get_pic(func)]), Normal())
            else:
