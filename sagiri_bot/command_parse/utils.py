    ...
    if await user_permission_require(group, member, 3):
        ...
        if level.isdigit():
            ...
            if member.id == target and level != 4 and await user_permission_require(group, member, 4):
                return MessageItem(MessageChain.create([Plain(text="主人想给自己降权，我该给他挂哪家医院的脑科，挺急的，在线等。")]), QuoteSource())
            if 1 <= level <= 2:
                if result := await orm.fetchone(select(UserPermission.level).where(UserPermission.group_id == group.id, UserPermission.member_id == target)):
                    if result[0] == 4:
                        if await user_permission_require(group, member, 4):
                            return MessageItem(MessageChain.create([Plain(text="当场给我整 NTR 是吧？")]),QuoteSource())
                        else:
                            return MessageItem(MessageChain.create([Plain(text="主人权限等级不可更改，若想进行修改请直接修改数据库。")]), QuoteSource())
                    if result[0] == 3:
                        if await user_permission_require(group, member, 4):
                            return await grant_permission_process(group.id, target, level)
                        else:
                            return MessageItem(MessageChain.create([Plain(text="权限不足，你必须达到权限等级4(主人)才可对超级管理员权限进行修改！")]), QuoteSource())
                    else:
                        return await grant_permission_process(group.id, target, level)
                else:
                    return await grant_permission_process(group.id, target, level)
                ...
                if await user_permission_require(group, member, 4):
                    ...
                    return MessageItem(MessageChain.create([Plain(text="格式错误！权限不足，你必须达到权限等级4(主人)才可对超级管理员进行授权！")]), Normal())
            else:
                return MessageItem(MessageChain.create([Plain("level 值非法！合法 level 值：1-3\n1: 用户\n2: 管理员\n3: 超级管理员")]), QuoteSource())
        else:
            return MessageItem(MessageChain.create([Plain("格式错误！\n使用方法：user -grant @user level[1-3]")]), QuoteSource())
    else:
        return MessageItem(MessageChain.create([Plain(text="权限不足，无法进行该操作。")]), QuoteSource())
...
async def execute_blacklist_append(member_id: int, group: Group, operator: Member) -> MessageItem:
    try:
        ...
        if await orm.fetchone(
            ...
            return MessageItem(MessageChain.create([Plain(text=f"{member_id} 已在本群黑名单中。")]), QuoteSource())
