...
# from sagiri_bot.utils import get_config, get_setting
#
...
#         #     #     return None
#         #     # else:
#         #     #     await update_user_call_count_plus(group, member, user_called_column_index[tfunc], user_called_name_index[tfunc])
#         #     #     if tfunc == "setu":
...
#         #     #         else:
#         #     #             return MessageItem(MessageChain.create([Plain(text="该功能已关闭，请阅读文档或者联系机器人管理员开启。")]), Normal())
...
#                 return MessageItem(MessageChain.create([await AdvancedImageSenderHandler.get_pic(func)]), Revoke())
#             elif r18_process == "flashImage":
#                 return MessageItem(MessageChain.create([(FlashImage.fromImage(await AdvancedImageSenderHandler.get_pic(func)))]), Normal())
#             elif r18_process == "noProcess":
#                 return MessageItem(MessageChain.create([await AdvancedImageSenderHandler.get_pic(func)]), Normal())
#             else:
#                 return MessageItem(MessageChain.create([Plain(text=f"Error: r18_process 值非法: {r18_process}")]), QuoteSource())
#         return MessageItem(MessageChain.create([await AdvancedImageSenderHandler.get_pic(func)]), Normal())
#
#     @staticmethod
